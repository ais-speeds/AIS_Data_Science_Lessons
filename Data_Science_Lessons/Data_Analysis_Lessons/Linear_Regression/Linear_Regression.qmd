...
The **multiple linear regression** (sometimes called a multivariable linear regression) extension accommodates multiple independent variables, expressed as:

$$Y=\beta_{0}+\beta_{1}X_{1}+\beta_{2}X_{2}+...+\beta_{n}X_{n}+\epsilon$$

which broadens the scope of applications to more complex datasets, by including more predictors. Linear regression serves as a powerful analytic tool in various domains, facilitating predictive modeling, hypothesis testing, and understanding the relationships within quantitative data.
...
-   `SUP_DOSE`: The dose (in mg) of oral iron supplement that each participant took, where 0 = None, 1 = 105 mg and 2 = 210mg
...
-   **The influential observations plot** (bottom-left panel) shows no cases falling outside of cutoff lines - suggesting that there are no potential influential points of concern.
-   **The normality of residuals** (bottom-right panel) plot shows that residuals are mostly normally distributed - suggesting that normality of residuals is met.
:::
...
Transferrin saturation is calculated from other iron parameters such as iron (umol/L) and transferrin (g/L), using the formula:

$$
\frac{3.982 \times \text{Iron (umol/L)}}{\text{Transferrin (g/L)}}
$$

Therefore, despite the VIF and tolerance statistics being acceptable, these variables are mathematically related to each other thus multicollinear.
:::
...
An example of k-folds cross validation is demonstrated below using the `tidymodels` framework:
